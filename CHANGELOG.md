...
- **OTP 27+ is now required.** `Crank.PurityTrace` uses `:trace.session_create/3` and the surrounding session-scoped tracing API, which is part of the new `:trace` module added in OTP 27. `Crank.Application` raises `CRANK_SETUP_002` at boot if running on OTP < 27. CI matrix runs only OTP 27+. See [`CRANK_SETUP_002`](guides/violations/CRANK_SETUP_002.md).
...
- **OTP guard (Stage 2).** `Crank.Application` boots with the OTP 27+ check, starts `Crank.TaskSupervisor` for Mode B worker tasks.
